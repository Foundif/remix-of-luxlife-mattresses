...
// 3. Solid Heart Icon for Wishlist
...
export function BottomNav() {
  ...
  const { count: bagCount, setOpen: setBagOpen } = useCart();
  const { count: wishlistCount, open: wishlistOpen, setOpen: setWishlistOpen } = useWishlist();
  const { user } = useAuth();

  // Auto-hide the bar on single product pages
      ...
      <div className="flex items-center justify-between gap-1 px-1">
        ...
        >
          <span className="relative inline-flex items-center justify-center">
            <HeartIcon className={cn("size-5 shrink-0 transition-all", wishlistCount > 0 && !wishlistOpen ? "text-red-500" : "")} />
            {wishlistCount > 0 && (
          ...
          <span
            className={cn(
              "text-[10px] tracking-tight transition-colors",
              wishlistOpen ? "font-bold text-ink" : "font-medium text-muted-foreground",
            )}
          >
            Wishlist
          </span>
